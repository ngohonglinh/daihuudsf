package net.feuapp.daihuudsf.crudmanager;
...
public class crudLocal extends SQLiteOpenHelper {
    ...
    final static String DATABASE_NAME="daihuudsf.db";
...
//    }
    ...
    public int DiemDD_SumDiemEater(long Eater){
        int result=0;
                //Cursor cursor = db.rawQuery("SELECT id, name FROM people WHERE name = ? AND id = ?", new String[] {"David", "2"});
        Cursor cursor = db.rawQuery("SELECT SUM(diemeater) FROM diemdd WHERE eater = ? AND chucvu <> ?", new String[] {String.valueOf(Eater), "Thuyền viên"});
        // nocty
        if (cursor.moveToFirst()) {
...
public long [] Ticket_getAmountInfo(long rkeyticket){
    ...
    public long TVCanCa_addTVCanCa(TVCanCa tvcanca) {
        //SQLiteDatabase db = sInstance.getWritableDatabase();
        ContentValues values = new ContentValues();
        ...
        long i=-1;
        try {
            i = db.insert("tvcanca", null, values);
        }catch(Exception e){
            Log.e(TAG, "addTVCanCa: "+e.getMessage().toString() );
        }finally {
            if (db!=null && db.isOpen()){
                //db.close();
            }
    ...
    public ArrayList<TVCanCa> TVCanCa_getTVCanCaByThuyenVien(long rkeyThuyenVien) {
        ArrayList<TVCanCa> arraylistTVCanCa = new ArrayList<>();
        String[] params = new String[]{String.valueOf(rkeyThuyenVien)};
        //SQLiteDatabase db = sInstance.getReadableDatabase();
        Cursor cursor=null;
        try {
            cursor = db.rawQuery("SELECT * FROM tvcanca WHERE rkeythuyenvien = ? ORDER BY id ASC", params);
            if (cursor.moveToFirst()) {
                do {
                    ...
                    arraylistTVCanCa.add(tvcanca);

                } while (cursor.moveToNext());
            }
            cursor.close();
            //db.close();
            return arraylistTVCanCa;

        }catch (Exception e){
            Log.e(TAG, "getTVCanCabyID: " + e.toString() );
            return null;
        }finally {
            if (cursor!=null && !cursor.isClosed()){
                cursor.close();
    ...
    public ArrayList<TVCanCa> TVCanCa_getAllTVCanCa() {
        ArrayList<TVCanCa> arraylistTVCanCa = new ArrayList<>();

        String selectQuery = "SELECT * FROM tvcanca";
    ...
    public boolean TVCanCa_ExitsServerKey(int serverkey) {
        String[] params = new String[]{ String.valueOf(serverkey)};
        
        //SQLiteDatabase db = sInstance.getReadableDatabase();
        Cursor cursor=null;
        try {
            cursor = db.rawQuery("SELECT * FROM tvcanca WHERE serverkey = ?", params);
            if (cursor.moveToFirst()) {
                return true;
            }else{
                return false;
            ...
            Log.e(TAG, "TVCanCa_ExitsServerKey: " + e.toString() );
            return false;
        }finally {
            if (cursor!=null && !cursor.isClosed()){
                cursor.close();
    ...
    public int TVCanCa_updateTVCanCa(TVCanCa tvcanca){
        //SQLiteDatabase db = sInstance.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put("serverkey", tvcanca.getServerkey());
        values.put("rkey", tvcanca.getRkey());
        ...
        values.put("updatetime",tvcanca.getUpdatetime());
        values.put("username", tvcanca.getUsername());
        int u =db.update("tvcanca",values,"id=?",new String[]{String.valueOf(tvcanca.getId())});
        if (db!=null && db.isOpen()){
            //db.close();
        }
        return u;
    ...
    public String TVCanCa_SumThanhTienByRkeyTV(long rkeyThuyenVien){
        long result=0;
        //SQLiteDatabase db = sInstance.getReadableDatabase();
        //Cursor cursor = db.rawQuery("SELECT id, name FROM people WHERE name = ? AND id = ?", new String[] {"David", "2"});
        Cursor cursor = db.rawQuery("SELECT SUM(thanhtien) FROM tvcanca WHERE rkeythuyenvien = ?", new String[] {String.valueOf(rkeyThuyenVien)});
        // nocty
        if (cursor.moveToFirst()) {
            result=cursor.getLong(0);
        }
    ...
    public int TVCanCa_deleteTVCanCa(long rkey){
        //SQLiteDatabase db = sInstance.getWritableDatabase();
        int d= db.delete("tvcanca","rkey=?",new String[] {String.valueOf(rkey)});
        if (db!=null && db.isOpen()){
            //db.close();
        }
        return d;
