...
import net.feuapp.daihuudsf.crudmanager.SyncCheck;
...
import net.feuapp.daihuudsf.customadapters.CustomAdapterDSTV;
import net.feuapp.daihuudsf.customadapters.CustomAdapterDebtBookDetail;
...
import static net.feuapp.daihuudsf.crudmanager.SyncCheck.IS_ADMIN;
import static net.feuapp.daihuudsf.crudmanager.SyncCheck.LOGIN_NAME;
import static net.feuapp.daihuudsf.crudmanager.SyncCheck.LOGIN_OK;
import static net.feuapp.daihuudsf.crudmanager.SyncCheck.MY_SERVER;
...
import static net.feuapp.daihuudsf.utils.utils.showSoftKeyboard;
...
public class DSTVActivity extends AppCompatActivity {
    ...
    private void gotoDebtBookDetail(int position){
        ...
        if (chiTietNoThuyenVien.size()>0) {
            ...
            Toast.makeText(DSTVActivity.this, "Thuyền viên này vẫn chưa ứng tiền", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public boolean onCreateOptionsMenu(Menu manu) {
        ...
        if (daChia){
            for (int i = 0; i < manu.size(); i++){
                if (manu.getItem(i).getItemId()!=R.id.id_dstv_canca){
        ...
        MenuItem atChia=manu.findItem(R.id.dstv_atChia);
        if (StringUtils.compareIgnoreCase(StringUtils.left(LOGIN_NAME,5),"admin")!=0){
            atChia.setVisible(false);
        }
        if (mShowMenuSave==1){
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch(itemId)  {
            ...
            case R.id.id_dstv_canca:
                ...
                return true;
            case R.id.dstv_atChia:
                ArrayList<DSTV>arrDSTV=  crudLocaldb.DSTV_getDSTVByChuyenBien(intentIdChuyenBien);
                if (arrDSTV.size()==0){
                    return true;
                }
                final AlertDialog.Builder dialogBuilder = new AlertDialog.Builder(this, R.style.AppTheme_MaterialDialogTheme);
                final EditText edtTTChia = new EditText(this);
                edtTTChia.setInputType(InputType.TYPE_CLASS_NUMBER);
                dialogBuilder.setTitle("Auto Tiền chia");
                dialogBuilder.setMessage("- Tự động tính Tiền chia cho DSTV dựa vào Tiền/Điểm \n\n"+
                        "- Nhập vào Tiền/Điểm");
                dialogBuilder.setView(edtTTChia);
                dialogBuilder.setCancelable(false);
                dialogBuilder.setPositiveButton("Update", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        long tpd = longGet(edtTTChia.getText().toString());
                        for (int i=0;i<arrDSTV.size();i++){
                            DSTV dstv=new DSTV();
                            dstv=arrDSTV.get(i);
                            dstv.setUpdatetime(getCurrentTimeMiliS());
                            dstv.setTienchia(String.valueOf(tpd*doubleGet(dstv.getDiem())));
                            dstv.setConlai(String.valueOf(longGet(tpd*doubleGet(dstv.getDiem())+"")-longGet(dstv.getTienmuon())-longGet(dstv.getTiencanca())));
                            crudLocaldb.DSTV_updateDSTV(dstv);
                        }
                        updateListDSTV();
                    }
                });
